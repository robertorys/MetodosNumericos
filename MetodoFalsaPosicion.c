...
/**
 * @brief Método de la falsa posición, regresa una raiz en un intervalo [a,b],
...
int metodoFalsaPosicion(terP *pol, int nTerm, double raiz) {
    ...
    printf("- Intervalo de busqueda: [%lf,%lf]\n", ai, bi);
    fprintf(out, "- Intervalo de busqueda: [%lf,%lf]\n", ai, bi);
    ...
    if(fa == 0) {
        printf("a = %lf, es raiz para la ecuacion\n", ai);
        fprintf(out, "a = %lf, es raiz para la ecuacion\n", ai);
        return ai;
    }
    if(fb == 0){
        printf("b = %lf, es raiz para la ecuacion\n", bi);
        fprintf(out, "b = %lf, es raiz para la ecuacion\n", bi);
        return bi;
    }
    ...
    while (true) {
        ...
        // Si el error relativo de f(pi); 10^-3 > eRelativo,
        // entonce pi es una solución aproximada.
        if(eRelativo < 0.001 || fp == 0){
